    ...
    return {
      ...
      upsertPatient: (patientFromDb) =>
        setData((prev) => {
          ...
          // ALTERAÇÃO: Padrão agora é 2 (Sem Pendências) se não especificado
          const targetListId = patientFromDb.listId ?? 2;
          let existingPatient: Patient | undefined;

          // Procura se já existe em qualquer lista
          ...
          if (!existingPatient) {
            const targetList =
              ...
              boardClone.lists[1]; // Fallback para lista 2 (index 1)
            if (targetList) {
      ...
      upsertReplacement: (row, initialFractions) =>
        setData((prev) => {
          ...
          // Move o paciente se necessário (ex: nova reposição aberta -> move para pendente)
      ...
      upsertFraction: (row) =>
        setData((prev) => {
          ...
          if (done >= foundReplacement.totalDuration) {
            ...
            // Caso alguém remova frações ou edite, pode reabrir
            foundReplacement.closed = false;
          }

          // Move o paciente se necessário (ex: fechou a última reposição -> move para sem pendências)
          if (affectedPatientId) {
            checkAndMovePatient(b, affectedPatientId);
          }
